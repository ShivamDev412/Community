...
const EventDetail = () => {
  ...
  const eventDate = moment(event?.event_date).format("ddd, MMM D YYYY");
  const eventTime = moment(event?.event_time, "HH:mm:ss").format("h:mm a");
  return (
    <section className="mb-10">
      ...
      <section className="flex justify-between flex-wrap w-11/12 sm:w-10/12 lg:w-9/12 2xl:w-6/12 mx-auto mt-6">
        <section className="w-full lg:w-[65%] h-auto">
          <EventDetailBannerImage image={event?.image} name={event?.name} />
