...
const EventCard: FC<{ data: EventType }> = ({ data }) => {
  const eventDate = moment(data?.event_date).format("ddd, MMM D");
  const eventTime = moment(data?.event_time, "HH:mm:ss").format("h:mm a");
  return (
    <Link to={`${RouteEndpoints.YOUR_EVENTS}/events/${data?.event_id}`}>
